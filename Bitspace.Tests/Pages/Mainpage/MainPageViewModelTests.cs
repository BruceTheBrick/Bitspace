...
public class MainPageViewModelTests : UnitTestBase<MainPageViewModel>
{
    ...
    public void Initialize_ShouldSetMenuItems()
    {
        ...
        Mocker.GetMock<IMainpageMenuItems>().Setup(x => x.GetMenuItems()).Returns(menuItems);

        //Act
        Sut.Initialize(It.IsAny<INavigationParameters>());

        //Assert
        Mocker.GetMock<IMainpageMenuItems>().Verify(x => x.GetMenuItems(), Times.Once);
    ...
    public void RefreshMenuItemsCommand_ShouldUpdateMenuItems()
    {
        //Arrange
        var menuItems = new ObservableCollection<MenuListItemViewModel>(MenuItemFactory.GetViewModels());
        Mocker.GetMock<IMainpageMenuItems>().Setup(x => x.ForceUpdateGetMenuItems()).Returns(menuItems);

        //Act
        Sut.RefreshMenuItemsCommand.Execute(null);

        //Assert
        Mocker.GetMock<IMainpageMenuItems>().Verify(x => x.ForceUpdateGetMenuItems(), Times.Once);
        Sut.MenuItems.Should().Equal(menuItems);
    }
    #endregion
}