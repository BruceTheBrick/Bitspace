...
public class ConnectFourScoringService : IConnectFourScoringService
{
    ...
    private List<List<Piece>> FindConsecutivePieces(IBoard board, Piece player, int numPieces)
    {
        ...
        foreach (var direction in directions)
        {
            for (var row = 0; row < board.Rows; row++)
            {
                ...
                {
                    var pieces = GetSection(board, row, col, direction.verticalVector, direction.horizontalVector,
                        numPieces);
    ...
    private IList<Piece> GetSection(IBoard board, int row, int column, int rowIncrement, int colIncrement,
        int numPieces)
    {
